...
\rustc is the most often used, and likely only, compiler for \rust.
...
but the process is rather expensive. It seems like a better idea to just 
...
splitting code is made of three parts, variable declaration modification,
function call modification, and assignment modification.
...
locality benefits actually come in. The simplest case is the one in which
...
References and arrays are handled the same as the simple case, so whenever we 
see a reference or an array containing a splittable structure, we split the declaration.
This process holds for any level of nestings. Examples of this
can be seen in Figure \ref{sec:splitting}.

\todo[inline]{This needs a lot of work, and might not even get implemented in time}
\todo[inline]{Vec doesn't work out of the box since sort\_by, sort\_by\_key, and 
binary\_search\_by exist.
Maybe should do something smart if the method has a function parameter that takes in
the structure? Maybe iterate and collect? Possibly create a function in Vec that 
takes in a member function and runs in on a tuple? There is potential this should be moved
to future work section. Maybe just abort compilation if these functions are called on
split Vec's? There should be a general solution to this.
Might have to require run time cost in these cases, iterate then zip then apply then
unzip and return}

\subsubsection{Structures}
\label{sec:param}
While in the general case we do not split a structure when it is parameterized by
a splittable structure, there are cases in which we want to split the base structure
instead of just tuplifying the splittable structure. It is very important
that we handle this correctly, since generic structures are very common in \rust. The most common,
and thankfully the easiest to deal with, is \vec. \vec is the \rust structure analygous
to the \cpp \texttt{std::vector}. It is used practically everywhere, so properly handling it
is extremely important. Most of the applications we would be targeting would be storing
their structures in a \vec, so not handling would result in structure splitting being
rather ineffective.

The current method of handling this is a whitelist of some of the most common containers
in the \rust standard library, with the ability for users to declare their container as 
split-friendly.
