...
\todo[inline]{should explain how structure splitting works in detail.  Cite related work, e.g. Zhong et al. PLDI 2004 and more recent papers citing the PLDI 2004 paper.}
...
input on a few of a trickier decisions. While requiring user input is not ideal
for a compiler pass, it is a significant first step in a fully automatic solution.

The project can be roughly separated into two parts, structure splitting and
\mir modification.
...
\todo[inline]{This part needs work, should mention the whole platform thing.}
...
\todo[inline]{This also needs some work}
...
Modifying \mir blocks so that structure splitting takes place is a multistep
process, the first of which is changing \mir local variable declarations in order to 
specify exactly how structures are split.
...
into three cases, no modification, in place modification, and split modification.
...
\subsection{In Place Modification}
Other types we only need to make in place changes to the type, meaning we do not
need to generate more variable declarations in order to process them.
...
% \section{User defined data structures}
