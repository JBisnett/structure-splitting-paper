...
\todo[inline, color = green!]{should explain how structure splitting works in detail}
...
Firstly, my library iterates through all declarations of local variables and
sees if any of them are splitable. If they are splitable, we duplicate the
declaration, replacing the type in each duplicate with a substructure.

Whether something is splitable or not is
sometimes a difficult decision to make, however.
...
arrays are split by checking if they contain or refer a splitable structure and
splitting if they do.

\todo{explain this more}
As long as we do not split more then one struct, tuples are also the same.

\todo{structures}
Structure are very easy in the case 
