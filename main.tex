...
%TODO: should explain how structure splitting works in detail
...
% My approach to solving this problem was to approach the challenge of structure
% splitting as more of a library then a true compiler pass. This is for two
% primary reasons. One, while \rust doesn't specifically say that structures can't
% be non-contiguous, but that is likely only because \rust doesn't have a standard
% yet. The only semi-official documentation on struct memory layouts for \rust, a
% short page in the \rust book on guidelines for unsafe \rust, doesn't even mention the possibility.
% Its hard to imagine that the casual user would accept it automatically then.
...
The rest of my work is the creation of a series of \mir passes that use the
sub-structures defined by the macro system in order to automatically switch
between 
...
being \vec. \v[autogobble]{rust}ec is the default \rust resizable array class, analagous to std::vector
