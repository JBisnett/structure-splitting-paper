...
declaration, replacing the type in each duplicate with a
substructure.\todo{Better description of what happens when there are nested types}
...
\todo{structures}
Structure are very easy in the case that the structure type is the split
structure, the structure is split as it is. The trouble comes when the structure
is not something that is splitable, but one of its generic parameters is. In
this case, 
